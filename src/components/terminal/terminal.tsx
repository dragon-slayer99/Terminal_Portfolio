...
const Terminal = () => {
  ...
  const executeCommand = useCallback((command: string): React.ReactNode => {
    ...
    switch (cmd) {
      ...
      case 'about':
        return <About />;
      case 'whoami':
        return <TypingEffect text={username} speed={30} />;
      case 'pwd':
        return <TypingEffect text={`/home/${username}`} speed={30} />;
      case 'skills':
        return <Skills />;
      case 'projects':
        return <Projects />;
      ...
      <div ref={scrollContainerRef} className="flex-grow p-4 overflow-y-auto">
        <div className="space-y-4">
          {history.map((item, index) => (
             <div key={item.id} style={{ opacity: index === history.length - 1 ? 1 : 0.8 }}>
                <CommandOutput command={item.command} showCommand={item.showCommand}>
                {item.output}
                </CommandOutput>
            </div>
          ))}
        ...
        <form onSubmit={handleFormSubmit} className="flex items-center mt-4">
          <label htmlFor="terminal-input" className="flex items-center">
            {prompt}
          </label>
